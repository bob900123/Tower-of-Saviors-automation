...
def LoopPage(page: ft.Page):
    ...
    sleep_num = 3
    show_text = ft.Text(
        "", size=15, weight=ft.FontWeight.W_600, color=ft.Colors.GREY_500
    )
    ...
    def build_lv():
        ...
        for idx, item in enumerate(items):
            if item["type"] == "click":
                ...
                dropdown = ft.Dropdown(
                    width=120,
                    options=[
                        ft.dropdown.Option(f"{name}") for name, x, y in table_data
                    ],
                    text_size=15,
                    border_width=0,
                ...
                c = ft.Row(
                    [ft.Text("拖曳"), ft.Column([first_dropdown, second_dropdown])]
                )
                listtile = ft.ListTile(
                    title=c,
                    leading=ft.Icon(ft.Icons.DONE_OUTLINED),
                ...
                file_col = ft.Column(
                    [
                        ft.TextButton(
                            item.get("file1") if item.get("file1") else "選擇檔案",
                            on_click=pick_file,
                            data=item["uuid"] + ",0",
                        ),
                        ft.TextButton(
                            item.get("file2") if item.get("file2") else "選擇檔案",
                            on_click=pick_file,
                            data=item["uuid"] + ",1",
                        ),
                    ],
                ...
                file_col = ft.Column(
                    [
                        ft.TextButton(
                            item.get("file1") if item.get("file1") else "選擇模板",
                            on_click=pick_file,
                            data=item["uuid"] + ",0",
                        ),
                        ft.TextButton(
        ...
        list_view = ft.ReorderableListView(
            controls=controls, on_reorder=on_reorder, key="reorderable-list"
        )
    ...
    sleep_bar = ft.SnackBar(content=ft.Text(f"{sleep_num} 秒後開始執行"), duration=3000)
        ...
        controls=[
            ...
            ft.Column(
                controls=[
                    ...
                    ft.ElevatedButton(
                        "延遲",
                        ft.Icons.BROWSE_GALLERY_OUTLINED,
                    ...
                    ft.ElevatedButton(
                        "相似",
                        ft.Icons.FIBER_SMART_RECORD_OUTLINED,
